 ...
 */
class LightsparkFuturesWalletClient constructor(config: ClientConfig) {
    private val coroutineScope = CoroutineScope(Dispatchers.IO)
    private val coroutinesClient = LightsparkCoroutinesWalletClient(config)
    ...
    fun loginWithJWT(accountId: String, jwt: String, storage: JwtStorage): CompletableFuture<LoginWithJWTOutput> =
        coroutineScope.future { coroutinesClient.loginWithJWT(accountId, jwt, storage) }
    ...
    suspend fun deployWallet(): CompletableFuture<Wallet> = coroutineScope.future { coroutinesClient.deployWallet() }
     ...
     */
    @Throws(LightsparkAuthenticationException::class, CancellationException::class)
    suspend fun terminateWallet(): CompletableFuture<Wallet> =
        coroutineScope.future { coroutinesClient.terminateWallet() }
    ...
    fun getWalletDashboard(
        ...
        coroutineScope.future { coroutinesClient.getWalletDashboard(numTransactions, numPaymentRequests) }
    ...
    fun payInvoice(
        ...
        coroutineScope.future { coroutinesClient.payInvoice(encodedInvoice, maxFeesMsats, amountMsats, timeoutSecs) }
     ...
     */
    fun getBitcoinFeeEstimate(): CompletableFuture<FeeEstimate> =
        coroutineScope.future { coroutinesClient.getBitcoinFeeEstimate() }
    ...
    suspend fun getLightningFeeEstimateForNode(
        ...
        coroutineScope.future { coroutinesClient.getLightningFeeEstimateForNode(destinationNodePublicKey, amountMsats) }
    ...
    suspend fun getLightningFeeEstimateForInvoice(
        ...
        coroutineScope.future { coroutinesClient.getLightningFeeEstimateForInvoice(encodedPaymentRequest, amountMsats) }
    ...
    fun createBitcoinFundingAddress(): CompletableFuture<String> =
        coroutineScope.future { coroutinesClient.createBitcoinFundingAddress() }

    /**
     * @return The current wallet if one exists, null otherwise.
     */
    @Throws(LightsparkAuthenticationException::class, CancellationException::class)
    fun getCurrentWallet(): CompletableFuture<Wallet?> = coroutineScope.future { coroutinesClient.getCurrentWallet() }
    ...
    fun requestWithdrawal(
        ...
        coroutineScope.future { coroutinesClient.requestWithdrawal(amountSats, bitcoinAddress) }
     ...
     * This call is only valid in test mode. You can simulate payment of the invoice by calling
